...
class FirewallRuleSearcher:
    """Firewall rule search system with deduplication of results."""
    ...
    def parse_xml_to_json(self, xml_file_path: str) -> List[Dict[str, Any]]:
        """Parse XML firewall rules to JSON."""
        try:
    ...
    def create_searchable_documents(self, rules: List[Dict[str, Any]]) -> List[Document]:
        """Convert JSON rules to documents for vector search."""
        documents = []
    ...
    def search_rules(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        """Semantic search with deduplication and GPT-3.5 analysis."""
        try:
            docs = self.vector_store.similarity_search(query, k=top_k * 3)  # Fetch more for deduplication

            # Deduplicate by rule_name (or another unique identifier)
            unique_rules = {}
            for doc in docs:
                rule_name = doc.metadata.get("rule_name", "UnknownRule")
                if rule_name not in unique_rules:
                    unique_rules[rule_name] = doc

            # Limit to top_k unique rules
            deduped_docs = list(unique_rules.values())[:top_k]
